...
def main():
    filepath = '/Users/apple/Demi/picture'  # 文件存储目录
    ...
    try:
        ...
        goto_next(bro)
        open_this_comment(bro)
        time.sleep(3)
        more_comment()
        
        comment_list = bro.find_elements_by_xpath('//div[@class="qolG5qEO"]')
        if comment_list != []:
            last_comment = comment_list[-1]
            bro.execute_script("arguments[0].scrollBy(0,50);", last_comment) 
...
def more_comment():
    ...
    pyautogui.scroll(random.randint(-10000,-8000))
...
def get_offset(count):
    ...
    if offset == 0:
        ...
        bili = 0.015
        result = offset-math.ceil(width*bili)
        print(result)
        return result//2      
